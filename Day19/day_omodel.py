...
BluePrintList = []
with open("input_day19.csv", "r") as data:
    for t in data:
        Line = t.strip().split()
        BlueNum, OreRO, ClayRO, ObsnRO, ObsnRC, GeodeROr, GeogeROb = int(Line[1][:-1]), int(Line[6]), int(Line[12]), int(Line[18]), int(Line[21]), int(Line[27]), int(Line[30])
...
def ParseMinute(Minute, Ore, Clay, Obsn, OreRo, ClayRo, ObsnRo, OreCost, ClayCost, ObsnCost, GeodeRoCost):
    ...
    if Ore >= OreCost and OreRo < MaxOreCost:
        BuildOre = ParseMinute(Minute-1, CollectedOre-OreCost, CollectedClay, CollectedObns, OreRo+1, ClayRo, ObsnRo, OreCost, ClayCost, ObsnCost, GeodeRoCost)
        GeodeScoreList.append(BuildOre)
    if Ore >= ClayCost and ClayRo < ObsnRoCostClay:
        BuildClay = ParseMinute(Minute-1, CollectedOre-ClayCost, CollectedClay, CollectedObns, OreRo, ClayRo+1, ObsnRo, OreCost, ClayCost, ObsnCost, GeodeRoCost)
        GeodeScoreList.append(BuildClay)
    if Ore >= ObsnRoCostOre and Clay >= ObsnRoCostClay and ObsnRo < GeodeRoCostObsn:
        BuildObsn = ParseMinute(Minute-1, CollectedOre-ObsnRoCostOre, CollectedClay-ObsnRoCostClay, CollectedObns, OreRo, ClayRo, ObsnRo+1, OreCost, ClayCost, ObsnCost, GeodeRoCost)
        GeodeScoreList.append(BuildObsn)
    if Ore >= GeodeRoCostOre and Obsn >= GeodeRoCostObsn:
        BuildGeode = ParseMinute(Minute-1, CollectedOre-GeodeRoCostOre, CollectedClay, CollectedObns-GeodeRoCostObsn, OreRo, ClayRo, ObsnRo, OreCost, ClayCost, ObsnCost, GeodeRoCost)
        BuildGeode += (Minute-1)
        return BuildGeode
    
    BuildNothing = ParseMinute(Minute-1, CollectedOre, CollectedClay, CollectedObns, OreRo, ClayRo, ObsnRo, OreCost, ClayCost, ObsnCost, GeodeRoCost)
